...
public class AwsS3Client {
	...
	public List<S3ObjectSummary> listAllObjects() {
		return amazonS3().listObjects(new ListObjectsRequest().withBucketName(awsS3Bucket)).getObjectSummaries();
	}

	public S3ObjectSummary getObjectByKey(String key) {
		return amazonS3().listObjects(new ListObjectsRequest().withBucketName(awsS3Bucket)).getObjectSummaries()
				.stream().filter(s3ObjectSummary -> s3ObjectSummary.getKey().equals(key)).findFirst().get();
	}
